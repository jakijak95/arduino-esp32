...
bool sigmaDeltaAttach(uint8_t pin, uint32_t freq)  //freq 1220-312500
{
  ...
  sdm_config_t config = {.gpio_num = (int)pin, .clk_src = SDM_CLK_SRC_DEFAULT, .sample_rate_hz = freq, .flags = {.invert_out = 0}};
...
bool sigmaDeltaWrite(uint8_t pin, uint8_t duty)  //chan 0-x according to SOC duty 8 bit
{
  ...
  if (bus != NULL) {
    ...
    if (err != ESP_OK) {
      log_e("sdm_channel_set_pulse_density failed with error: %d", err);
      return false;
    }
    return true;
  } else {
